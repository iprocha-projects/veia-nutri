...
export function ClientAppView({ client }: { client: any }) {
 ...
 if (res.ok) {
 alert('Check-in enviado com sucesso ao seu nutricionista!')
 window.location.reload()
 }
 } catch (e) {
 console.error(e)
 ...
 if (res.ok) {
 alert('Foto de evolução enviada!')
 window.location.reload()
 }
 } catch (e) {
 console.error(e)
 ...
 <p className="text-xs text-[#71808A]">Nutri: {client.professional?.user?.name || 'Dra. Amanda'}</p>
 </div>
 </div>
 </div>
 ...
 className="w-full text-xs p-2.5 rounded-lg border border-[#E2E8EE]"
 >
 ...
 <img src={p.photoUrl} alt="Evolução" className="w-full h-36 rounded-lg object-cover" />
 <span className="text-[10px] text-[#71808A] block text-center">
